...
import { editEvent, getEventById } from '@/app/actions/event'
...
export default function EventEditPage({ params }: { params: { id: string } }) {
  ...
  const [changedLogo, setChangedLogo] = useState<boolean>(false)
  const [changedBackground, setChangedBackground] = useState<boolean>(false)
    ...
    ;(async function () {
      try {
        ...
        formik.setValues({
          ...
          slogan: response.tagLine,
          date: new Date(response.date),
          location: response.location,
          time: response.time,
          max_tickets: response.tickets || 0,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let logoSrc = formik.values.logo
      let backgroundSrc = formik.values.background
      if (changedBackground) {
        backgroundSrc = await handleUpload(background!, 'background')
      }
      if (changedLogo) {
        logoSrc = await handleUpload(logo!, 'logo')
      }
      ...
      const eventResponse = await editEvent(id, event, space?.id || '')
      ...
      <main className="flex flex-grow justify-center overflow-hidden py-2 md:py-4 bg-white rounded-md">
        ...
        >
          <div className="flex flex-col gap-4 px-0.5 py-8 overflow-y-auto flex-grow max-w-lg">
            <h4 className="text-2xl font-medium">Edição de evento</h4>
            <Input
