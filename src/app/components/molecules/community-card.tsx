...
export function CommunityCard(props: { community: CommunityLite }) {
  ...
  return (
    <Card
      className={`flex flex-col h-fit space-y-2 md:flex-row w-full hover:bg-gray-50 justify-between p-4 rounded-2xl gap-2 cursor-pointer`}
      ...
      <CardContent className="flex flex-wrap md:flex-nowrap gap-6 p-0 ">
        ...
        <article className="flex flex-col justify-between h-full gap-2">
          <section className="flex flex-col space-y-2">
            ...
            <p className={`flex text-primary`}>{description}</p>
          ...
          <Link
            href={`/community/${id}`}
            className={`w-full md:w-fit whitespace-pre-line md:flex`}
          >
            <Button
              variant={'default'}
