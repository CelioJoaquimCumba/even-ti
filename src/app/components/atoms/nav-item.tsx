...
export default function NavItem ({selected = false, label='nav-item', path}: NavItemProps) {
    ...
    return (
        <Link href={path}>
            <li className={`flex  px-6 py-3 rounded-lg ${selected || state === 'hover' ? "bg-secondary" : "bg-background"}`} onMouseEnter={() => state === 'default' && setState('hover')} onMouseLeave={() => state === 'hover' && setState('default')}>
                {label}
            </li>
        </Link>
    )
